...
export async function POST(request: Request) {
  ...
  if (!supabase) return NextResponse.json({ error: "La base de datos aún no está configurada. Puedes entrar en la demo." }, { status: 503 });

  let query = await supabase.from("app_users").select("id, username, display_name, password_hash, is_active").eq("username", identifier).maybeSingle();
  if (!query.data && identifier.includes("@")) query = await supabase.from("app_users").select("id, username, display_name, password_hash, is_active").eq("email", identifier).maybeSingle();
